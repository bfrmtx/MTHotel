...
| time Domain | symbol | frequency domain | symbol |
| --- | --- | --- | --- |
| sampling rate | f~s~ = 1/Δt | (sampling) bandwidth | bw = f~s~ /2 |
| samples | N | spectral lines | N/2 + 1 (+1 == DC part) |
| window length | wl = T = N * Δt | frequency resolution [Hz] (bandwith) | $Δf = \frac{bw}{N/2} = \frac{ f_s }{N}$ |
