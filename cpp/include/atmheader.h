...
public:
    ...
    bool read(const fs::path &filename = "", const bool close_after_read = false) {
        if (filename.empty() && this->filename.empty()) return false;
        else if (!filename.empty()) this->filename = filename;
        ...
        if (close_after_read) this->file.close();
        return true;
    }
    ...
    bool write(const bool close_after_write = false) {
        if (!sizeof(this->filename.filename())) return false;
