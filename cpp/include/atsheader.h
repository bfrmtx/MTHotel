...
public:
  ...
  std::string get_ats_filename(const size_t &run = 99) const {
    ...
    if (this->header.sample_rate < 4096.5)
      atsfile += "_BL_";
    else
      atsfile += "_BH_";
  ...
  size_t ats_read_ints_doubles(std::vector<T> &ints_doubles) {
    ...
    if (!this->count_ats_read_ints_doubles)
      std::cerr << "converting " << this->filename << std::endl;
    else
      std::cout << ".";
  ...
  double calc_lsb_from_dbl_vec_mV(const std::vector<double> &ddata) {
    ...
    this->header.lsbval = lsb;
    return this->header.lsbval;
  }
...
public:
  ...
  std::string what;
  comp_if_equal(const std::shared_ptr<atsheader> &lhs, const std::string &what) :
      lhs(lhs), what(what) {
    if (this->what == "lsbval")
...
public:
  Greater(double th, std::string wh) :
      _than(th), what(wh) {
  }
