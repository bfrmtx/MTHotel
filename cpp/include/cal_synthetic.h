...
std::vector<std::complex<double>> gen_trf_mfs06e_chopper_on_template(const std::vector<double> &freqs) {
    ...
    for (const auto &f : freqs) {
        ...
        cal[i++] = 0.8 * (  (p1 / ( 1. + p1)) * (1. / (1. + p2)) * (1. / (1. + p4))  );
    }
    return cal;
}

std::vector<std::complex<double>> gen_trf_mfs06e_chopper_off_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
        ...
        cal[i++] = 0.8 * (  (p1 / ( 1. + p1)) * (1. / (1. + p2)) * (p3 / (1. + p3)) * (1. / (1. + p4)) );    }
    return cal;
}

std::vector<std::complex<double>> gen_trf_mfs07e_chopper_on_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
        ...
        cal[i++] = 0.64 * (  (p1 / ( 1. + p1)) * (1. / (1. + p2))  * (1. / (1. + p4)) );
    }
    return cal;
}

std::vector<std::complex<double>> gen_trf_mfs07e_chopper_off_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
        ...
        std::complex<double> p3 = (f / 0.72) * im;
        std::complex<double> p4 = (f / 50000.0) * im;
        cal[i++] = 0.64 * (  (p1 / ( 1. + p1)) * (1. / (1. + p2)) * (p3 / (1. + p3))  * (1. / (1. + p4)) );
    }
    return cal;
}

std::vector<std::complex<double>> gen_trf_mfs07_chopper_on_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
...
std::vector<std::complex<double>> gen_trf_mfs07_chopper_off_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
...
// Raklin Geomag-01
std::vector<std::complex<double>> gen_trf_fgs02_template(const std::vector<double> &freqs) {
    std::vector<std::complex<double>> cal(freqs.size());
    for (size_t i = 0; i < freqs.size(); ++i) {
        cal[i] = std::complex<double>(7.5000E-04, 0.0);
    }
    return cal;
}
...
std::vector<std::complex<double>> gen_trf_fgs03e_template(const std::vector<double> &freqs) {
    std::vector<std::complex<double>> cal(freqs.size());
    for (size_t i = 0; i < freqs.size(); ++i) {
        cal[i] = std::complex<double>(1.0000E-04, 0.0);
    }
    return cal;
}
// bartington mag-04, low noise, 70 000 nT, never sold yet

std::vector<std::complex<double>> gen_trf_fgs04e_template(const std::vector<double> &freqs) {

    std::vector<std::complex<double>> cal(freqs.size());
    for (size_t i = 0; i < freqs.size(); ++i) {
        cal[i] = std::complex<double>(1.4300E-04, 0.0);
    }

    return cal;
}
// should reach E = 50mV / nT at 10 kHz
std::vector<std::complex<double>>  gen_trf_shft02e_template(const std::vector<double> &freqs) {
    std::complex<double> im (0.0, 1.0);
    size_t i = 0;
    std::vector<std::complex<double>> cal(freqs.size());
    for (const auto &f : freqs) {
 ...
 */
std::vector<std::complex<double>> gen_trf_adb_08e_hf(const std::vector<double> &freqs, const ADU &hp_filter, const double &gain_1 = 1.0) {

    // input resisitivity doe not play a role
    std::complex<double> im (0.0, 1.0);
    std::vector<std::complex<double>> cal(freqs.size());
    size_t i = 0;
    for (const auto f : freqs) {
 ...
 */
std::vector<std::complex<double>>  gen_trf_adb_07e_hf(const std::vector<double> &freqs, const ADU &hp_filter, const double &gain_1 = 1.0, const double &gain_2 = 1.0) {
 ...
 * \return
 */
std::vector<std::complex<double>> gen_trf_adb_08e_lf(const std::vector<double> &freqs, const ADU &radio_filter, const ADU &lp_filter, const ADU &input_div,
...
std::vector<std::complex<double>> gen_trf_adb_10e_lf(const std::vector<double> &freqs, const ADU &input_div,
                                                     const double &resistance, const double &gain_1 = 1.0) {
    ...
    for (auto &f : freqs) {
        ...
        if  (input_div == ADU::div_8) p2 = (f / 7.8E3) * im;
        else if (input_div == ADU::div_1) p2 = (f / ( 0.159 / ((resistance + 200.) * 6.8E-9) ) ) * im;
...
std::vector<std::complex<double>>  gen_trf_adb_07e_lf(const std::vector<double> &freqs, const ADU &radio_filter, const ADU &lp_filter,
...
std::vector<std::complex<double>> gen_trf_adb_07e_mf(const std::vector<double> &freqs, const double &sample_freq,
