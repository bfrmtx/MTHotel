...
int main(int argc, char *argv[]) {
  ...
  while (argc > 1 && (l < unsigned(argc)) && *argv[l] == '-') {
    ...
    if (marg.compare("-chats-chopper") == 0) {
      ...
      if (mstr::contains(str_chopper, "on", false))
        chopper = ChopperStatus::on;
      if (mstr::contains(str_chopper, "off", false))
        chopper = ChopperStatus::off;
  ...
  if (chats) {
    ...
    for (auto &atsh : atsheaders) {
      ...
      if (change_chopper) {
        if (atsj->header["sample_rate"] < 513)
          atsj->header["chopper"] = ChopperStatus::on;
      } else
        atsj->header["chopper"] = chopper;
    ...
    for (const auto &calib : calibs) {
      ...
      if (xxcal.back()->cal_type() == CalibrationType::mtx)
        xxcal.back()->tasks_todo(false, false, false, false, true);
    }
