...
inline std::vector<std::string> gplt_blues = {"web-blue", "royalblue", "steelblue", "light-blue", "blue", "dark-blue", "midnight-blue", "medium-blue", "skyblue", "slateblue"};
inline std::vector<std::string> gplt_reds = {"red", "dark-red", "light-red", "orange-red", "brown", "brown4", "sandybrown"};
inline std::vector<std::string> gplt_greens = {"web-green", "dark-spring-green", "light-green", "green", "dark-green", "spring-green", "forest-green", "sea-green", "dark-olivegreen", "seagreen", "greenyellow"};
// can't see is on screen inline std::vector<std::string> gplt_yellows = {"yellow", "dark-yellow", "yellow4", "light-goldenrod", "dark-goldenrod", "gold", "goldenrod"};
inline std::vector<std::string> gplt_oranges = {"orange", "dark-orange", "orange-red", "coral", "orangered4"};
inline std::vector<std::string> gplt_browns = {"brown", "saddlebrown", "sienna", "tan", "peru", "chocolate", "burlywood", "wheat", "khaki"};
inline std::vector<std::string> gplt_blacks = {"black", "grey30", "grey40", "grey50", "grey60", "grey70", "grey80", "grey90", "grey100"};
...
public:
  ...
  void set_base_color(const std::string &base_color) {
    ...
    } else if (this->base_color == "brown") {
      this->colors = gplt_browns;
      this->colors = gplt_oranges;
...
public:
  ...
  std::string default_color(const std::pair<std::string, std::string> spectra_type, const std::string formats = "") const {
    ...
    // Use the static color vectors defined above
    const std::vector<std::string> &colors_Hx = gplt_blues;
    const std::vector<std::string> &colors_Hy = gplt_reds;
    const std::vector<std::string> &colors_Hz = gplt_greens;
    const std::vector<std::string> &colors_Ex = gplt_oranges;
    const std::vector<std::string> &colors_Ey = gplt_browns;
    const std::vector<std::string> &colors_Ez = gplt_blacks;
  ...
  void ascci_blocks() {
    if (this->xaxis.size() && (this->xaxis.size() == this->yaxis.size())) {
      ...
      while (!this->xaxis.empty()) {
        ...
        data << "e" << std::endl;
        if (data.tellp() > 1) {
          fprintf(this->plotHandle, "%s", data.str().c_str());
        }
        this->xaxis.pop();
        this->yaxis.pop();
      }
