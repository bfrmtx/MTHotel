...
#include "iterator_templates.hpp"
...
T median(const std::vector<T> &v) {
  ...
  if (w.size() % 2 == 0) // array is even
    ...
    med = w[w.size() / 2]; // integer division should not round up
  return med;
}
...
double coherence(const std::vector<T> &x, const std::vector<T> &y) {
  ...
  double mean_nominator = 0.0;
  double mean_x = 0.0;
  double mean_y = 0.0;

  for (size_t i = 0; i < x.size(); ++i) {
    mean_nominator += std::abs(x[i] * y[i]);
    mean_x += real(x[i] * std::conj(x[i]));
    mean_y += real(y[i] * std::conj(y[i]));
  }

  // Calculate averages
  mean_nominator /= double(x.size());
  mean_x /= double(x.size());
  mean_y /= double(x.size());

  // Square nominator for coherence formula
  mean_nominator = std::pow(mean_nominator, 2.0);

  // Calculate coherence
  double coh = mean_nominator / (mean_x * mean_y);

  // Return square root for coherency (as opposed to coherence)
  return std::sqrt(coh);
}
...
inline std::vector<double> simple_smoothing(const std::vector<double> &in, int n_avg) {
  if (n_avg < 2)
    n_avg = 2;
  if (in.size() < size_t(n_avg))
    return in;

  std::vector<double> out(in.size());
  double n_avg_d = double(n_avg) + 1.0; // we use n_avg + 1 to get the correct average, so if n_avg is 3, we use 4 values for the average
  size_t half = n_avg / 2;
  // first and last n_avg/2 values are not smoothed; we start at half and end at size - half
  // a) copy the first half values
  for (size_t i = 0; i < half; ++i) {
    out[i] = in[i];
  }
  for (size_t i = half; i < in.size() - half; ++i) {
    double sum = 0.0;
  ...
  // b) copy the last half values
  for (size_t i = in.size() - half; i < in.size(); ++i) {
    out[i] = in[i];
  }
  return out;
}
...
// * \return index vector of v referring to entries with no duplicate
// */
// template <typename T>
