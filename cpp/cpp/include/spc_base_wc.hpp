...
public:
  ...
  void add_channel(const std::pair<std::string, std::string> name, const std::shared_ptr<channel> chan1, const std::shared_ptr<channel> chan2 = nullptr, const bool overwrite = false) {
    ...
    if (chan2 == nullptr) {
      this->ch_map.emplace(name, std::make_pair(std::make_shared<channel>(chan1), nullptr));
      return;
    }
    if (chan1->get_sample_rate() != chan2->get_sample_rate()) {
      throw std::runtime_error("spc_base::add_channel: channel sample rates do not match for channels " + name.first + " and " + name.second);
    }
    // both channels are given
  ...
  std::vector<std::pair<std::shared_ptr<channel>, std::shared_ptr<channel>>> generate_auto_spectra_channels(const bool verbose = false) {
    ...
    for (const auto &ch_pair : this->ch_map) {
      auto chan1 = ch_pair.second.first;
      auto chan2 = ch_pair.second.second;
      if (chan1 == nullptr) {
        throw std::runtime_error("spc_base::prepare_auto_spectra: channel 1 is nullptr");
      }
      std::string name_in1 = chan1->channel_type;
      if (is_E(name_in1) && chan1->is_emap && !chan1->is_remote) {
        name_in1 = "E" + name_in1;
      ...
      // second must be empty for auto spectra
      if (chan2 != nullptr) {
        throw std::runtime_error("spc_base::generate_auto_spectra_channels: channel 2 must be nullptr for auto spectra");
      }
      auto_cross_spectra_channels.emplace_back(std::pair<std::shared_ptr<channel>, std::shared_ptr<channel>>(chan1, chan1));

      if (verbose) {
        std::cout << "spc_base::generate_auto_spectra_channels: prepared auto spectra " << name_in1 << ", " << name_in1 << std::endl;
      }
    }
    return auto_cross_spectra_channels;
  }
  ...
  void save_to_ascii(const std::filesystem::path &top_dir, const bool create = false) const {
    std::shared_lock lock(spc_lock);
    if (this->empty()) {
      throw std::runtime_error("spc_base::save_to_ascii: no spectra to save");
    }
    if (this->ch_map.empty()) {
      throw std::runtime_error("spc_base::save_to_ascii: no channels available to generate file names");
    }
    if (this->size() != this->ch_map.size()) {
      throw std::runtime_error("spc_base::save_to_ascii: number of spectra and channels do not match");
    }
    if (this->bin_extension.empty()) {
      throw std::runtime_error("spc_base::save_to_ascii: bin extension is not set, cannot save binary file");
    }
    if (this->ascii_extension.empty()) {
      throw std::runtime_error("spc_base::save_to_ascii: ascii extension is not set, cannot save ascii file");
    }
    // type should be double or std::complex<double>, not vector of vectors
    if constexpr (is_std_vector<T>::value) {
      throw std::runtime_error("spc_base::save_to_ascii: T must not be a vector (should be double or std::complex<double>)");
    }
    // check that T is a double or std::complex<double>
    using T_no_cvref = std::remove_cv_t<std::remove_reference_t<T>>;
    if constexpr (!(std::is_same_v<T_no_cvref, double> ||
                    std::is_same_v<T_no_cvref, std::complex<double>>)) {
      throw std::runtime_error("spc_base::save_to_ascii: T must be double or std::complex<double>");
    }
    if (create) {
      std::filesystem::create_directories(top_dir);
    }
    if (!std::filesystem::exists(top_dir) || !std::filesystem::is_directory(top_dir)) {
      throw std::runtime_error("spc_base::save_to_ascii: directory " + top_dir.string() + " does not exist");
    }
    for (const auto &spc : this->ch_map) { // we iterate over the map
      std::shared_ptr<channel> chan1;
      std::shared_ptr<channel> chan2;
      std::shared_ptr<fftw_freqs> fft_freqs;
      std::filesystem::path filename = top_dir;
      // save the channels as json , we only need the path and call the channel write_header function
      if (spc.second.first != nullptr) {
        chan1 = spc.second.first;
        chan1->set_dir(top_dir);
        chan1->write_header();
        filename /= chan1->get_filepath_wo_ext().filename();
        if (fft_freqs == nullptr) {
          fft_freqs = spc.second.first->fft_freqs;
        }
      }
      if (spc.second.second != nullptr) {
        chan2 = spc.second.second;
        chan2->set_dir(top_dir);
        if (chan1 != chan2) {
          chan2->write_header();
        }
        if (!spc.second.first) {
          filename /= chan2->get_filepath_wo_ext().filename();
        } else {
      ...
      filename += "." + this->ascii_extension; // add the extension for ascii file
      auto xspc = std::pair<std::string, std::string>(spc.first.first, spc.first.second);
      auto data_ptr = this->at(xspc);
      auto freqs = fft_freqs->get_frequencies();
      // save as ascii .datfa or .datfc depending on the data type
      fdirs::save_vector_to_ascii_f(filename, freqs, *data_ptr);
    }
  }

  void load_from_rundir(const std::filesystem::path &top_dir, const std::string &ascii_extension = "") {
    std::unique_lock lock(spc_lock);
    // we must ensure that this class is freshly created.
    if (!this->empty()) {
      throw std::runtime_error("spc_base::load_from_rundir: spectra map is not empty, cannot load data");
    }
    ...
    this->ascii_extension = ascii_extension;
  ...
  void set_extension(const std::string &ext_type) {
    ...
    this->ascii_extension = "dat" + ext_type;
  }
  std::string get_bin_extension() const {
    std::shared_lock lock(spc_lock);
    return this->bin_extension;
  }
  std::string get_ascii_extension() const {
    std::shared_lock lock(spc_lock);
    return this->ascii_extension;
  }
