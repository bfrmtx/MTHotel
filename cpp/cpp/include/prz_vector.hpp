#ifndef PARZEN_VECTOR_HPP
#define PARZEN_VECTOR_HPP
...
template <typename T>
void nsmooth(const std::vector<T> &data, const std::vector<size_t> &indices, const size_t nlines, std::vector<T> &result) {
  result.reserve(indices.size());
  // for each selected frequency iterate over freqs until we find the selected frequency or >=
  ...
  if (nlines == 1) {
    ...
    for (const auto &idx : indices) {
      ...
      if (n < 0) {
        ...
        err_str << "::n is negative: " << n;
        throw std::runtime_error(err_str.str());
      }
      for (n = n; n < idx + half + 1; ++n) { // we need to add 1 to include the last element
        sum += data[n];
      }
      result.push_back(sum / nlines);
    }
  }
...
template <typename T>
void nsmooth_t(const std::shared_ptr<std::vector<T>> &data, const std::vector<size_t> &indices, const size_t nlines, std::shared_ptr<std::vector<T>> &result) {
  nsmooth(*data, indices, nlines, *result);
}
...
inline constexpr std::size_t parzen_vector(const std::vector<double> &freqs, const std::vector<double> &target_freqs, const double &prz_radius,
