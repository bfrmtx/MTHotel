...
public:
  ...
  fftw_freqs(const std::shared_ptr<fftw_freqs> &rhs) {
    if (rhs == nullptr) {
      std::ostringstream err_str(__func__, std::ios_base::ate);
      err_str << ":: rhs is nullptr";
      throw std::runtime_error(err_str.str());
    }
    // *this = fftw_freqs(rhs->sample_rate, rhs->wl, rhs->rl);
    ...
    this->target_freqs = rhs->target_freqs;
    this->selected_freqs = rhs->selected_freqs;
    this->parzendists = rhs->parzendists;
    this->prz_radius = rhs->prz_radius;
  ...
  double get_f(const size_t &idx) const {
    return (double(idx) * (this->sample_rate / double(this->wl)));
  }

  double get_f_target(const size_t &idx) const {
    return this->target_freqs[idx];
  }

  double get_f_selected(const size_t &idx) const {
    return this->selected_freqs[idx];
  }
  ...
  size_t set_target_freqs(const std::vector<double> &fin, const double &prz_radius, bool simple_avg = false) {
    ...
    if (simple_avg) {
      this->nlines_avg = size_t(prz_radius);
      // check if cast was successful
      if (this->nlines_avg == 0 || this->nlines_avg == SIZE_MAX) {
        std::ostringstream err_str(__func__, std::ios_base::ate);
        err_str << ":: nlines_avg must be > 0";
        throw std::runtime_error(err_str.str());
      }

      // check if nlines_avg is odd
      if (this->nlines_avg % 2 == 0) {
        std::ostringstream err_str(__func__, std::ios_base::ate);
        err_str << ":: nlines_avg must be > 0";
        throw std::runtime_error(err_str.str());
      }
      this->prz_radius = 0.0;
      size_t half = this->nlines_avg / 2; // in case of 5 we have 2 on each side
      // we use the idx_range to get the frequencies
      // auto fv = this->get_frequencies();
      // for (const auto &x : fv) {
      //   std::cout << "f:" << x << std::endl;
      //  }
      for (size_t i = this->idx_range.first + half; i < this->idx_range.second - half; ++i) {
        double fft_f = (double(i) * (this->sample_rate / double(this->wl)));
        for (const auto &f : fin) {
          //  std::cout << "f:" << f << " fft_f:" << fft_f << std::endl;
          if (this->almost_equal(f, fft_f)) {
            //    std::cout << "selected f:" << f << " fft_f:" << fft_f << std::endl;
            this->smooth_index.push_back(i - this->idx_range.first); // we need the index in the fftresult
            this->target_freqs.push_back(fft_f);
            this->selected_freqs.push_back(fft_f); // it should be guaranteed that the selected freqs are in the target freqs; that is different from parzen, we we do this later
          }
        }
      }
    } else {
      // use the classic parzen window
      ...
      this->nlines_avg = 0;
      this->prz_radius = prz_radius; // ? must be < 1 ?
    }
  ...
  std::vector<double> get_selected_frequencies() const {
    if (this->selected_freqs.empty()) {
      throw std::runtime_error("get_selected_frequencies: selected_freqs is empty");
    }
