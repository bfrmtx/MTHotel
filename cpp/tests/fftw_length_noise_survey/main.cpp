...
*/
  ...
  for (size_t i = 0; i < channels.size(); ++i) {
    ...
    try {
      ...
      if (inner_range)
        f = fft_freqs[i]->get_frequency_slice(innerouter.get_inner());
      else
        f = fft_freqs[i]->get_frequency_slice(innerouter.get_outer());
      if (inner_range)
        v = trim_by_index<double>(raws.at(i)->get_abs_sa_spectra(channel_type), fft_freqs[i]->get_index_slice());
      else
        v = raws.at(i)->get_abs_sa_spectra(channel_type);
      double rms = 0;
  ...
  try {
    for (size_t i = 0; i < channels.size(); ++i) {
      ...
      if (fft_freqs.at(i)->get_wl() != fft_freqs.at(i)->get_rl()) {
        ...
        gplt->set_xy_lines(fft_freqs.at(i)->get_frequencies(), v, label, 1, "lc rgbcolor \"blue\"");
    }
  } catch (const std::runtime_error &error) {
    std::cerr << error.what() << std::endl;
