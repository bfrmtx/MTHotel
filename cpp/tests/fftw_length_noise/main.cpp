...
int main() {
  ...
  for (size_t i = 0; i < channels.size(); ++i) {
    ...
    if (delta_f == 1 && !i)
      gplt->set_xy_points(fft_freqs.at(i)->get_frequencies(), vs[i], label.str(), 2, "pt 6");
    else if (delta_f == 1 && i)
      gplt->set_xy_linespoints(fft_freqs.at(i)->get_frequencies(), vs[i], label.str(), 2, 2, "pt 9");
    else
      gplt->set_xy_lines(fft_freqs.at(i)->get_frequencies(), vs[i], label.str(), 1);
  }
