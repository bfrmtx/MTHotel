...
int main(int argc, char* argv[])
{
    ...
    for (size_t loop = 0; loop < 3; ++loop) {   // for three examples with / wo close frequencies
        ...
        if (loop) gplt_ts->cmd << "set title 'TS Data \\@ 50 + 50.5 Hz + small noise'" << std::endl;
        else gplt_ts->cmd << "set title 'TS Data \\@ 50 Hz'" << std::endl;
        ...
        if (loop) gplt->cmd << "set title 'FFT \\@ 50 + 50.5 Hz'" << std::endl;
        else gplt->cmd << "set title 'FFT 50 Hz'" << std::endl;
