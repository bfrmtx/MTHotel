...
struct calibration {
  ...
  calibration(std::string sensor, uint64_t serial, uint64_t chopper) :
      sensor(sensor), serial(serial), chopper(chopper) { ; }
};
...
struct s_find_serial {
  uint64_t serial = 0;
  uint64_t chopper = 0;
  s_find_serial(const std::shared_ptr<calibration> &lhs) :
      serial(lhs->serial), chopper(lhs->chopper) { ; }
...
int main() {
  ...
  i = 0;
  for (const auto &cal : cals) {
    if (find_serial(find_me, cal))
      std::cout << cal->serial << " auto loop chopper on found at " << i << std::endl;
    i++;
  }
  std::cout << std::endl;
