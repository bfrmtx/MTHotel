...
double math_vector::sel_mean(std::vector<double>::const_iterator xcbeg, std::vector<double>::const_iterator xcend,
                          std::vector<bool>::const_iterator selected_cbeg, std::vector<bool>::const_iterator selected_cend,
                          const size_t min_valid_elements, const int control) {
...
std::ostream& operator<<(std::ostream& out,
    ...
    for ( auto it = a.variance_data.cbegin(); it != a.variance_data.cend(); ++it ) {
        if (it->second > 0) {
            out  << std::setw(22) << std::left <<  g_stat::gauss_names.at(it->first) << "    " << it->second << std::endl;
        }
        else {
            out  << std::setw(22) << std::left <<  g_stat::gauss_names.at(it->first) << "   " << it->second << std::endl;
...
statmap two_pass_variance_cplx::variance(std::vector<std::complex<double>>::const_iterator xcbeg, std::vector<std::complex<double>>::const_iterator xcend,
    ...
    this->d_max = *xcbeg;
    for (; xcbeg != xcend; ++xcbeg) {
        if (std::fabs(*xcbeg) < std::fabs(this->d_min)) this->d_min = std::fabs(*xcbeg);
        if (std::fabs(*xcbeg) > std::fabs(this->d_max)) this->d_max = std::fabs(*xcbeg);
        sdiffs = *xcbeg - this->d_mean;
        this->d_dmean += std::fabs(sdiffs);
        this->ep += sdiffs;
        this->d_variance += (moment = sdiffs * sdiffs); // m2
        this->d_skewness += (moment *= sdiffs);         // m3
        this->d_kurtosis += (moment *= sdiffs);         // m4
...
void two_pass_variance_cplx::calc_all() {
    ...
    this->variance_data[g_stat::d_n_x] = this->d_n ;
    this->variance_data[g_stat::sum_x] = std::fabs(this->d_sum);
    this->variance_data[g_stat::min_x] = std::fabs(this->d_min);
    this->variance_data[g_stat::max_x] = std::fabs(this->d_max);
    this->variance_data[g_stat::mean_x] = std::fabs(this->d_mean);
    this->variance_data[g_stat::range_x] = std::fabs(this->d_range);
    this->variance_data[g_stat::variance_population_x] = std::fabs(this->d_variance_population);
    this->variance_data[g_stat::variance_x] = std::fabs(this->d_variance);
    this->variance_data[g_stat::stddev_population_x] = std::fabs(this->d_stddev_population);
    this->variance_data[g_stat::stddev_x] = std::fabs(this->d_stddev);
    this->variance_data[g_stat::skewness_x] = std::fabs(this->d_skewness);
    this->variance_data[g_stat::kurtosis_x] = std::fabs(this->d_kurtosis);
}
