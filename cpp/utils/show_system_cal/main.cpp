...
int main(int argc, char* argv[])
{
    ...
    for (const auto &str :what) {
        if (str == "gen_trf_adb_08e_lf") {
            for (j = 0; j < 3; ++j) {
                ...
                labels.back() << "ADB 08e LF, LF-RF-1, res: " << resistance << " Ohm," << "gain 1: " << gain_1;
    ...
    if (!files) gplt->cmd << "set terminal qt size 2048,1600 enhanced" << std::endl;
    ...
    for (const auto &str :what) {
        if (str == "gen_trf_adb_08e_hf") {
            for (j = 0; j < 3; ++j) {
                ...
                bvec::cplx2_vap(trfs.back(), ampls, phs, true);
                labels.emplace_back(std::stringstream());
                labels.back() << "ADB 08e LF, HF, res: " << resistance << " Ohm," << "gain 1: " << gain_1;
