...
def plot_time_series_quick(atss_files_in, start, wl, title=None, xlabel=None, ylabel=None, legend=None, save=None, marker=None):
    ...
    # if marker is not None, marker must same dimension like atss_files_in, so two maker arrays for two files for example
    if marker is not None:
        if type(atss_files_in) is list:
            if len(marker) != len(atss_files_in):
                raise ValueError("Marker array must have same length as atss_files_in")
        else:
            if not hasattr(marker, '__len__') or len(marker) == 0:
                raise ValueError("Marker must be a non-empty array when atss_files_in is a single file")
    ...
    for atss_file in atss_files:
        ...
        if marker is not None:
            # marker is a red dot, x=marker, y=ats_data
            # Get the marker indices for this file
            if type(atss_files_in) is list:
                file_index = atss_files.index(atss_file)
                marker_indices = marker[file_index]
            else:
                marker_indices = marker

            # Plot markers as red dots
            for idx in marker_indices:
                plot_idx = idx - start  # Convert absolute position to relative position in the window
                if 0 <= plot_idx < len(atss_data):
                    ax.plot(plot_idx, atss_data[plot_idx], 'ro')
            ax.plot(atss_data)
        else:
            ax.plot(atss_data)
...
def plot_time_series_diff(atss_file_in_1, atss_file_in_2, start, wl, title=None, xlabel=None, ylabel=None, legend=None, save=None, marker=None):
    """
    Plot time series from two ATSS files for comparison.
    Marker is a single array that applies only to the first file.
    """
    # Create list of exactly two files
    atss_files = [atss_file_in_1, atss_file_in_2]
    ...
    for i, atss_file in enumerate(atss_files):
        atss_data = atss.read_data(atss_file, start, wl)
        
        # Plot the first file with markers if provided
        if i == 0 and marker is not None:
            # Plot markers as red dots on first file only
            # marker indices are absolute positions in the full file
            # atss_data starts at 'start' position, so adjust marker indices
            for idx in marker:
                plot_idx = idx - start  # Convert absolute position to relative position in the window
                if 0 <= plot_idx < len(atss_data):
                    ax.plot(plot_idx, atss_data[plot_idx], 'ro', markersize=8)
            ax.plot(atss_data, label=f'File 1' if legend is None else legend[i] if legend and i < len(legend) else f'File 1')
        else:
            # Plot without markers for second file or when no markers
            ax.plot(atss_data, label=f'File {i+1}' if legend is None else legend[i] if legend and i < len(legend) else f'File {i+1}')
