...
import sys
import os
from lib.examin_st import extract_log_messages_aduX, write_html_table_logs
...
if __name__ == "__main__":
  ...
  input_dir = './input'
  error_msg_db_file = "./lib/adu_error_msg_definitions_ADUX.sql3"  # error messages SQLite database file  

  # for ADUX we only have one file in input directory, called systemLog.db
  sqlite_db_file = os.path.join(input_dir, 'systemLog.db')
  
  # Ensure output directory exists
  os.makedirs('./output', exist_ok=True)
  
  # Extract log messages and write to HTML
  log_messages = extract_log_messages_aduX(sqlite_db_file, error_msg_db_file)
  if log_messages:
    output_file = os.path.join('./output/', os.path.basename(sqlite_db_file).replace('.db', '.html'))
